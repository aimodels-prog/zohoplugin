...
Version 3.3.2 also rewrites controls inside stored oversized responses before fragmentation, and tests complete reconstruction using the old list tool. Schema errors and failed report verification now appear in account-scoped operational diagnostics with a reference and release ID. Configured Contabo deployments run a public, read-only report acceptance check across all five entities and roll back on failure. See [release acceptance and diagnostics](ASSURANCE.md#release-acceptance-and-diagnostics-v332); these delivery checks do not replace independently approved finance references or a real ChatGPT conversation test.
